...
if [ -f "CTF/docker_hardening/harden_docker.sh" ]; then
    chmod +x CTF/docker_hardening/harden_docker.sh
    CTF/docker_hardening/harden_docker.sh
    echo "[+] Docker hardening completed successfully."
else
    echo "[-] Docker hardening script not found at CTF/docker_hardening/harden_docker.sh"
    exit 1
fi
